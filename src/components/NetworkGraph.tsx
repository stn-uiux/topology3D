      ...
      {clickedNode && tooltipPos && (
        <div
          ...
          style={{
            ...
            top: `${tooltipPos.y}px`,
            transform: `translate(${tooltipPos.x > window.innerWidth - 350 ? 'calc(-100% - 15px)' : '15px'}, ${tooltipPos.y > window.innerHeight - 350 ? 'calc(-100% - 15px)' : '15px'})`
          }}
        >
          <div
            className="bg-[#161920]/95 backdrop-blur-md rounded-xl p-4 text-[#cbd5e1] shadow-2xl min-w-[280px] max-w-[320px] border"
            style={{
              borderColor: clickedNode.severity && clickedNode.severity !== 'normal'
                ? (SEVERITY_COLORS[clickedNode.severity]?.color || 'rgba(255, 255, 255, 0.08)')
          ...
          >
            ...
            <div className="flex justify-between items-start mb-2">
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-widest">
                {clickedNode.isGroupNode ? 'GROUP NODE' : (clickedNode.isDeviceNode ? 'DEVICE NODE' : 'PORT INTERFACE')}
              </span>
              <button
                onClick={() => {
                  ignoreHoverUntilRef.current = Date.now() + 300;
                ...
                className="text-gray-500 hover:text-gray-300 text-xs transition-colors p-0.5"
                title="닫기"
              >
            ...
            {/* Title */}
            <div className="text-[15px] font-bold text-slate-100 mb-2 leading-snug">
              {clickedNode.label.replace('\n', ' ')}
            </div>

            {/* Alarm Banner */}
            {clickedNode.severity && clickedNode.severity !== 'normal' ? (
              <div
                className="px-3 py-2 rounded-r-md mb-2 text-left"
                style={{
                  background: hexToRgba(SEVERITY_COLORS[clickedNode.severity]?.color || '#f97316', '0.12'),
                  borderLeft: `3px solid ${SEVERITY_COLORS[clickedNode.severity]?.color || '#f97316'}`
                }}
              >
                <div
                  className="text-[11px] font-bold uppercase"
                  style={{ color: SEVERITY_COLORS[clickedNode.severity]?.color || '#f97316' }}
                >
                  {clickedNode.severity === 'critical' ? 'CRITICAL (치명)' :
                    clickedNode.severity === 'major' ? 'MAJOR (경고)' :
                      clickedNode.severity === 'minor' ? 'MINOR (주의)' : 'WARNING (알람)'}
                </div>
                <div className="text-[11px] text-slate-200 mt-0.5 font-medium leading-relaxed">
                  {ALARM_CAUSES[clickedNode.id] || '하위 장애 전파 검출 및 라인 다운 경보'}
                </div>
              </div>
            ) : (
              <div className="border-l-2 border-emerald-500 bg-emerald-500/10 px-3 py-1.5 rounded-r-md mb-2">
                <div className="text-[11px] font-bold text-emerald-400 uppercase">SYSTEM HEALTHY</div>
              </div>
            )}

            {/* Metadata Details */}
            <div className="text-[11px] text-slate-400 flex flex-col space-y-0.5 mt-2.5">
              {clickedNode.ipAddr && (
                <div>• IP 주소: <span className="font-mono text-slate-200 ml-1">{clickedNode.ipAddr}</span></div>
              )}
              {clickedNode.deviceGroupId && (
                <div>• 그룹 ID: <span className="text-slate-200 ml-1">{clickedNode.deviceGroupId}</span></div>
              )}
            </div>

            {/* Interactive Drilldown Actions */}
            {clickedNode.severity && clickedNode.severity !== 'normal' && (
              <>
                {/* 1뎁스/그룹 노드 장애인 경우 */}
                {clickedNode.isGroupNode && (() => {
                  // Find all faulty devices in this group (using raw data.nodes and mock Alarms)
                  const faultyDevices = data.nodes.filter(n => {
                    if (n.deviceGroupId !== clickedNode.deviceGroupId || !n.isDeviceNode) return false;

                    // Case 1: Device itself has a node alarm
                    if (mockNodeAlarms[n.id] && mockNodeAlarms[n.id] !== 'normal') return true;

                    // Case 2: Any interface of this device has a physical line alarm
                    const deviceInterfaces = data.nodes.filter(inf => inf.parentDeviceId === n.id && inf.isInterfaceNode);
                    const interfaceIds = new Set(deviceInterfaces.map(inf => inf.id));

                    const hasLineAlarm = Object.keys(mockLinkAlarms).some(alarmKey => {
                      const [port1, port2] = alarmKey.split('--');
                      return interfaceIds.has(port1) || interfaceIds.has(port2);
                    });
                  ...
                  if (faultyDevices.length > 0) {
                    return (
                      <div className="mt-3.5 border-t border-slate-800 pt-2.5">
                        <div className="text-[11px] text-slate-500 font-bold uppercase tracking-wider mb-1.5">하위 장애 발생 장비 ({faultyDevices.length}):</div>
                        <div className="flex flex-col space-y-1.5 max-h-[140px] overflow-y-auto pr-1">
                          {faultyDevices.map(dev => {
                            let devSeverity = mockNodeAlarms[dev.id] || 'normal';
                            if (devSeverity === 'normal') {
                              const deviceInterfaces = data.nodes.filter(inf => inf.parentDeviceId === dev.id && inf.isInterfaceNode);
                              const interfaceIds = new Set(deviceInterfaces.map(inf => inf.id));
                              Object.entries(mockLinkAlarms).forEach(([alarmKey, severity]) => {
                                const [port1, port2] = alarmKey.split('--');
                                if (interfaceIds.has(port1) || interfaceIds.has(port2)) {
                                  devSeverity = getMaxSeverity(devSeverity, severity);
                                }
                              });
                            }
                            const sevColor = SEVERITY_COLORS[devSeverity]?.color || '#f97316';
                            return (
                              <div key={dev.id} className="flex items-center justify-between bg-[#1d1f27] hover:bg-[#232733] p-1.5 rounded-md border border-[#2d3748]/50 transition-colors">
                                <div className="text-[11px] font-bold flex items-center pr-2 truncate" style={{ color: sevColor }}>
                                  <span className="w-1.5 h-1.5 rounded-full mr-1.5 shrink-0 animate-pulse" style={{ background: sevColor, boxShadow: `0 0 4px ${sevColor}` }}></span>
                                  <span className="truncate">{dev.label.replace('\n', ' ')}</span>
                                </div>
                                <button
                                  onClick={() => {
                                    if ((window as any).focusNodeById) {
                                      (window as any).focusNodeById(dev.id);
                                    }
                                    setClickedNode(null);
                                    setTooltipPos(null);
                                  }}
                                  className="p-1 bg-[#ea580c]/10 hover:bg-[#ea580c] text-[#fdba74] hover:text-white border border-[#ea580c]/30 hover:border-transparent rounded transition-all cursor-pointer shrink-0 flex items-center justify-center"
                                  title="해당 장비로 즉시 이동"
                                >
                ...
                {/* 2뎁스/장비 노드 장애인 경우 */}
                {clickedNode.isDeviceNode && (() => {
                  const faultyPorts = visibleData.nodes.filter(n => n.parentDeviceId === clickedNode.id && n.isInterfaceNode && n.severity !== 'normal');
                  if (faultyPorts.length > 0) {
                    return (
                      <div className="mt-3.5 border-t border-slate-800 pt-2.5">
                        <div className="text-[11px] text-slate-500 font-bold uppercase tracking-wider mb-1.5">하위 장애 포트 회선 ({faultyPorts.length}):</div>
                        <div className="flex flex-col space-y-1.5 max-h-[140px] overflow-y-auto pr-1">
                          {faultyPorts.map(port => {
                            const sevColor = SEVERITY_COLORS[port.severity]?.color || '#06b6d4';
                            return (
                              <div key={port.id} className="flex items-center justify-between bg-[#1d1f27] hover:bg-[#232733] p-1.5 rounded-md border border-[#2d3748]/50 transition-colors">
                                <div className="text-[11px] font-bold flex items-center pr-2 truncate" style={{ color: sevColor }}>
                                  <span className="w-1.5 h-1.5 rounded-full mr-1.5 shrink-0 animate-pulse" style={{ background: sevColor, boxShadow: `0 0 4px ${sevColor}` }}></span>
                                  <span className="truncate">[포트] {port.interfaceName || '물리 포트'}</span>
                                </div>
                                <button
                                  onClick={() => {
                                    if ((window as any).focusNodeById) {
                                      (window as any).focusNodeById(port.id);
                                    }
                                    setClickedNode(null);
                                    setTooltipPos(null);
                                  }}
                                  className="p-1 bg-[#0891b2]/10 hover:bg-[#0891b2] text-[#67e8f9] hover:text-white border border-[#0891b2]/30 hover:border-transparent rounded transition-all cursor-pointer shrink-0 flex items-center justify-center"
                                  title="해당 포트로 즉시 이동"
                                >
                                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                                </button>
